...
- PR gate status: draft PR #21 has passing CI, database/RLS, Edge Deno, E2E, performance, security, and Windows-worker checks. CodeQL and dependency review are deliberately retained as required checks but fail before analysis because GitHub reports Code Scanning and GitHub Advanced Security are disabled for this private repository. This is an operator-owned repository setting; no gate has been bypassed.
