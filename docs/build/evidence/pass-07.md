...
- After the production secrets were provisioned, deployment-hardening PR #19 (`c98838a`) switched function bundling to Supabase API mode with three bounded attempts. Staging run `30858448987` then passed Worker, migrations, all Edge Functions, and smoke; production run `30858615653` passed secret preflight, Worker, migrations, all Edge Functions, and safe smoke.
- Final local `corepack pnpm verify:ci` passed on 2026-08-03. Independent HTTPS checks returned `200` for both `/login` endpoints; both responses included the expected CSP. The remaining acceptance work is the operator-owned production onboarding checklist and immutable acceptance record, which require configured provider accounts and real MFA rather than synthetic test identities.
