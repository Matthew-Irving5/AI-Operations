-- A personal GitHub evidence sync reads private repository metadata and must
-- be explicitly authorised with the same one-time fresh-MFA gate used by the
-- other sensitive connection operations.
...
alter table public.mfa_action_gates
  ...
  check (action_key in (
    ...
    'worker_device_register', 'worker_device_revoke', 'digital_scan_create',
    'finance_configure', 'finance_import', 'github_sync'
  ));
