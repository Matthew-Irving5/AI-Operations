...
import { APPROVED_GOOGLE_SCOPES, hasExactGoogleScopes } from '../_shared/google-sync.ts';
import {
...
const encrypt = async (value: string) => {
  ...
  return `${btoa(String.fromCharCode(...nonce))}.${btoa(String.fromCharCode(...ciphertext))}`;
};
...
async function recordFailure(failure: GoogleOAuthFailure, context: FailureContext): Promise<void> {
...
async function respondFailure(
  ...
  if (origin) {
    ...
    target.searchParams.set('correlation_id', failure.correlation_id);
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (result.error) {
      return respondFailure(
        failure(correlationId, {
          ...
          retryable: true,
          details: { database_error: databaseCode(result.error.code) ?? 'unknown' },
        }),
        503,
      );
    }
    ...
    if (providerError) {
      return respondFailure(
        failure(correlationId, {
          ...
          stage: 'callback_input',
          reason:
            providerError === 'access_denied' ? 'user_denied_consent' : 'provider_returned_error',
    ...
    const clientSecret = env('GOOGLE_OAUTH_CLIENT_SECRET', 'GOOGLE_CLOUD_CLIENT_SECRET');
    ...
    try {
      ...
      const code =
        ...
        ['token_encryption_unconfigured', 'token_encryption_key_invalid'].includes(error.message)
          ? error.message
      ...
      return respondFailure(
        failure(correlationId, {
          ...
          details: {
            ...
            ...(providerTokenError ? { provider_error: providerTokenError } : {}),
          },
        }),
        422,
        context,
      ...
      return respondFailure(
        failure(correlationId, {
          ...
          details: {
            ...
            unexpected_scopes: grantedScopes.filter((scope) => !approvedScopeSet.has(scope)),
          },
        }),
        422,
        context,
      ...
      return respondFailure(
        failure(correlationId, {
          ...
          details: {
            ...
            ...(providerProfileError ? { provider_error: providerProfileError } : {}),
          },
        }),
        502,
        context,
    ...
    const profileEmail =
      typeof profileBody.email === 'string' ? profileBody.email.toLowerCase() : null;
    if (!profileEmail) {
      await revokeAccessToken(tokens.access_token!);
      return respondFailure(
        failure(correlationId, {
          ...
          details: { profile_verified_email: profileBody.verified_email === true },
        }),
        403,
        context,
      );
    ...
    if (profileEmail !== allowedEmail) {
      ...
      return respondFailure(
        failure(correlationId, {
          ...
          details: { profile_verified_email: true, allowlisted_account_match: false },
        }),
        403,
        context,
      );
    ...
    if (connection.error || !connection.data) {
      return respondFailure(
        failure(correlationId, {
          ...
          retryable: true,
          details: { database_error: databaseCode(connection.error?.code) ?? 'unknown' },
        }),
        500,
        context,
      );
    ...
    if (credential.error || !credential.data) {
      ...
      return respondFailure(
        failure(correlationId, {
          ...
          reason: 'database_write_failed',
          retryable: true,
          details: { database_error: databaseCode(credential.error?.code) ?? 'unknown' },
        }),
        500,
        context,
      );
    ...
    target.searchParams.set('correlation_id', correlationId);
    return Response.redirect(target.toString(), 302);
  } catch {
    return respondFailure(
      failure(correlationId, {
