...
export default async function MfaPage({ searchParams }: { searchParams: Promise<{ returnTo?: string }> }) {
  ...
  const returnTo = requestedReturnTo?.startsWith('/') && !requestedReturnTo.startsWith('//') ? requestedReturnTo : '/overview';
  return (
    <main style={{ maxWidth: 480, paddingTop: '12vh' }}>
      ...
      {verifiedFactor ? <MfaChallenge factorId={verifiedFactor.id} returnTo={returnTo} /> : <MfaChallenge returnTo={returnTo} />}
    </main>
  );
}
